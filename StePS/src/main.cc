...
int IC_FORMAT;
int OUTPUT_FORMAT;
int OUTPUT_TIME_VARIABLE;
double MIN_REDSHIFT;
int REDSHIFT_CONE;
int HAVE_OUT_LIST;
double TIME_LIMIT_IN_MINS;
int H0_INDEPENDENT_UNITS;
double *out_list;
double *r_bin_limits;
int out_list_size;
...
int numtasks, rank;
int N_mpi_thread;
int ID_MPI_min, ID_MPI_max;
MPI_Status Stat;
int BUFFER_start_ID;
REAL *F_buffer;

// Ewald space variables
int e[2202][4];
int el;

// Integration and error variables
REAL x4, err, errmax, ACC_PARAM;
double h, h_min, h_max, t_next;
double a_max, t_bigbang;
...
REAL M_tmp;
REAL M_min;
REAL mass_in_unit_sphere;
REAL rho_part;
REAL beta, particle_radii;
REAL w[3];
REAL SOFT_CONST[8];
REAL *SOFT_LENGTH;

// Cosmological parameters
double Omega_b, Omega_lambda, Omega_dm, Omega_r, Omega_k, Omega_m,
    ...
    double wa;
#elif COSMOPARAM==-1
    char EXPANSION_FILE[1024];
    int N_expansion_tab;
    int expansion_index;
    double **expansion_tab;
    int INTERPOLATION_ORDER;
#endif
double rho_crit;
double a, a_start, a_prev, a_tmp;
double T, delta_a, Omega_m_eff;
...
int main(int argc, char *argv[])
{
  ...
  MPI_Bcast(M,N,MPI_FLOAT,0,MPI_COMM_WORLD);
    ...
    if(rank == 0)
    {
        ...
        rho_part = M_min/(4.0*pi*pow(particle_radii, 3.0) / 3.0);
    ...
    if(COSMOLOGY == 1)
    {
        ...
        if(COMOVING_INTEGRATION == 1)
        {
        printf("Initial time:\t\tt_start = %.10f Gy\nInitial scale factor:\ta_start = %.8f\nMaximal scale factor:\ta_max   = %.8f\n\n", T*UNIT_T, a, a_max);
        }
