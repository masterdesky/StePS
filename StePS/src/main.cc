...
int main(int argc, char *argv[])
{
    ...
    if(rank == 0)
    {
        ...
        printf("|  ____) | ||  __/ |     ____) |\t\t\t\t\t\t\t\t|\n");
        printf("| |_____/ \\__\\___|_|    |_____/ \t\t\t\t\t\t\t\t|\n");
        printf("| StePS %s\t\t\t\t\t\t\t\t\t\t\t|\n", PROGRAM_VERSION);
        printf("| (STEreographically Projected cosmological Simulations)\t\t\t\t\t|\n");
        printf("+-----------------------------------------------------------------------------------------------+\n");
        printf("| Copyright (C) 2017-2024 Gabor Racz\t\t\t\t\t\t\t\t|\n");
        printf("| Jet Propulsion Laboratory, California Institute of Technology | Pasadena, CA, USA\t|\n");
        printf("| Department of Physics of Complex Systems, Eotvos Lorand University | Budapest, Hungary\t|\n");
        printf("| Department of Physics & Astronomy, Johns Hopkins University | Baltimore, MD, USA\t|\n");
        printf("| \t\t\t\t\t\t\t\t\t\t\t\t|\n");
        printf("| Build date: %s\t\t\t\t\t\t\t|\n", BUILD_DATE);
    ...
    //The rank=0 thread reads the paramfile, and bcasts the variables to the other threads
    if(rank == 0)
    {
    ...
    if(rank == 0)
    {
        ...
        if(COSMOLOGY == 1)
        {
            if(COMOVING_INTEGRATION == 1)
            {
                printf("Timestep %d, t=%.8f Gy, h=%.3f My, a=%.8f, H=%.8f km/s/Mpc, z=%.8f\n", 
                       t, T * UNIT_T, h * UNIT_T * 1000.0, a, Hubble_param * UNIT_V, 1.0 / a - 1.0);
