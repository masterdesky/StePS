...
REAL DE = (REAL) H0*H0*Omega_lambda;  //Dark energy density
...
#ifdef PERIODIC
//force calculation with multiple images
            ...
            #pragma omp for schedule(dynamic,chunk)
                for(i=ID_min; i<ID_max+1; i++) {
                    for(j=0; j<N; j++)
                    {
                        ...
                        if(fabs(dx)>0.5*L) { dx = dx-L*dx/fabs(dx); }
                        if(fabs(dy)>0.5*L) { dy = dy-L*dy/fabs(dy); }
